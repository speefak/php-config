...
#TODO#TODO listen auswahl wie bei apache mods
	...
	if [[ -n $Monochrome ]]; then
		printf "$PHPVersionsListParsed" | sed 's/^/ /'
	else
		printf "$PHPVersionsListParsed" | sed 's/ .*installed://'
	fi
