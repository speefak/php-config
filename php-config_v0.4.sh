...
#------------------------------------------------------------------------------------------------------------
	...
	OptionVarList="
		...
		ShowInstalledPHPVersion;-si
		ShowAvailablePHPVersion;-sa
	"
...
usage() {
	...
	printf " -sa			  => show available php versions \n"
	printf " -si			  => show installed php versions \n"
...
show_available_PHP_Versions ()  {
	printf "\n"
	apt-cache search php | grep -w "^php[[:digit:]].[[:digit:]] " | cut -d "-" -f1 | sed 's/php/php /;s/^/  /'
	printf "\n"
}
#------------------------------------------------------------------------------------------------------------
